...
const mongoose = require("mongoose"),
  GenericProcedure = require("@utils/utilgenericfunctions"),
  { _responseWrapper } = require("@utils/utilresponse"),
...
exports.removeCacheFN = async key => {
  if (key) {
    ...
    let removed_cache_object = await GenericProcedure._baseRemove(
      CacheSchema,
      args,
      "HardRemove"
    );
...
exports.getCacheFN = async () => {
  ...
  let all_cache = await GenericProcedure._baseFetch(CacheSchema, args);
  if (!all_cache.status)
